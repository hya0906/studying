...
def evaluate_network(files, args, identity_list):
	# GPU: active speaker detection by pretrained model
	s = ASD()
	face_model = FaceNet().cuda()
	...
	for file in tqdm.tqdm(files, total = len(files)):
		...
		if len(identity_list) == 0 or numpy.max(sim) < 0.5:
			...
			identity_list = numpy.concatenate((identity_list, face_id_vec[None]), axis=0)
		else:
			identity_number = numpy.argmax(sim)
		identity_result.append(identity_number)
	...
	return allScores, identity_result
...
def visualization(tracks, scores, args):
	...
	for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):
		...
		for face in faces[fidx]:
			...
			#######
			xx = int(face['x'] - face['s']) #&
			yy = int(face['y'] - face['s']) #&
			ww = int(2 * face['s']) #&
			hh = int(2 * face['s']) #&
...
def load_model_and_data(): #&
    ...
    net = net.cuda()
    weight = torch.load(f'.\\checkpoint\\no_freeze_v2_0.00025+(1)\\test_epoch21_84.5176_191.pth')
...
def main():
	...
	# Before execute, you should download arcface weight file from (https://1drv.ms/u/s!AhMqVPD44cDOhkPsOU2S_HFpY9dC),
	# and place the weight file at: weight/model_ir_se50.pth
	...
	with open(identity_result_path, 'wb') as fil:
		pickle.dump(identity_result, fil)
