...
def get_train_loader(batch_size, num_worker):
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size= batch_size, pin_memory = pin_memory, shuffle=False, num_workers=num_worker)

    validloader = torch.utils.data.DataLoader(
        validset, batch_size=batch_size, pin_memory=pin_memory, shuffle=False, num_workers=num_worker)
...
def main_worker(gpu, ngpus_per_node):
    ...
    testloader = torch.utils.data.DataLoader(testset, batch_size=16, shuffle=False, num_workers=ngpus_per_node)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        torch.save(net.state_dict(), os.path.join("./weight/112_Adam_80_0.001_bs32,16", 'epoch-{}_model_state_dict.pt'.format(epoch)))
    end = time.time()
    sec = end - start
