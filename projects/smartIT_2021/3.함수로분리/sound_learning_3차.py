...
class Test:
    ...
    def get_feature(self, fName, flag):
        global saved_model
        main_dir = fName
        head, tail = os.path.split(main_dir)
        sub_dir = os.listdir(head)
        if flag == 2:
    ...
    '''
    def get_audioFeature(self, fName):
        global saved_model
        main_dir = fName
        head, tail = os.path.split(main_dir)
        print(main_dir)
    ...
    def get_videoFeather(self, fName):
        global saved_model
        # 영상
        #main_dir = os.getcwd()
        main_dir = fName
        head, tail = os.path.split(main_dir)
        sub_dir = os.listdir(head)
        print("head, tail, sub_dir", head, "    ",tail,"    ",sub_dir)
        # try:
        #     if type(float(tail)) != float:
        #         pass
        # except ValueError:
        #     os.rename(tail, "05.mp4")
        #head, tail = os.path.split(main_dir)
        #print("head, tail, sub_dir", head, "    ", tail, "    ", sub_dir)
        clip = mp.VideoFileClip("./video/" + tail)
        length = clip.duration
