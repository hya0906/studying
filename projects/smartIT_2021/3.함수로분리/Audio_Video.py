...
front_path = "C:/Users/USER/Desktop/files/haarcascade_frontalface_default.xml"
profile_path = 'C:/Users/USER/anaconda3/pkgs/libopencv-3.4.2-h20b85fd_0/Library/etc/haarcascades/haarcascade_profileface.xml'
eye_path = "C:/Users/USER/anaconda3/pkgs/libopencv-3.4.2-h20b85fd_0/Library/etc/haarcascades/haarcascade_eye.xml"
...
class Audio_Video:
    ...
    def after_rec(self): #녹음후처리
        try:
            ...
            print('Error: Creating directory. ' + "./sound")
        os.chdir("./sound")

        waveFile = wave.open(f"{WAVE_OUTPUT_FILENAME}{self.count}"+".wav", 'wb')
    ...
    def video_play(self): # cap에 따라 실시간,동영상 가능
        ...
        faces = face_detection1.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30)) #정면
        if faces is None:
            faces = face_detection2.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30)) #측면
    ...
    def detect(self, gray, frame):
        ...
        self.faces = face_detection1.detectMultiScale(gray, scaleFactor=1.05, minNeighbors=5, minSize=(100, 100),
        ...
        for (x, y, w, h) in self.faces:
            ...
            for i, (eye_x, eye_y, eye_w, eye_h) in enumerate(eyes):
                ...
                # cv.circle(face_color, (eye_x, eye_y), 2, (0, 0, 255), 2)
                # cv.circle(face_color, (eye_x+eye_w, eye_y+eye_h), 2, (0, 0, 255), 2)
                # cv.circle(face_color, (eye_x + int(eye_w/2), eye_y + int(eye_h/2)), 2, (0, 0, 255), 2)
                if i == 0:
        ...
        try:
            ...
            self.new_img = Image.fromarray(self.frame_raw)
            self.new_img = np.array(self.new_img.rotate(direction * -angle))

        except ZeroDivisionError as e:
            pass

        return self.new_img  # frame