...
def parse_audio_files(parent_dir,sub_dirs , file_ext="*.wav"): #parent_dir,sub_dirs,file_ext="*.wav"
...
class Test:
    ...
    def audio_feature(self):
        ...
        print("done")
        np.save('Xrelearning', features)  # 저장
        # one hot encoding labels
        labels = one_hot_encode(labels)
        np.save('yrelearning', labels)
    ...
    def video_feature(self): #맨 처음 데이터 특징추출하기 위한 부분-npy로 저장
        ...
        np.save('frelearning', features)
        np.save('lrelearning', labels)

    def musical_feature(self): #맨 처음에 npy파일을 만들때만 사용
        # 뮤지컬
    ...
    def main(self, value):
        if value == 'yes':
            self.get_feature_save()
            Xrelearning = np.load('Xrelearning.npy') #audio
            Yrelearning = np.load('yrelearning.npy') #audio
            xrelearning = np.load('frelearning.npy') #videio
            yrelearning = np.load('lrelearning.npy') #video
            ...
            y = np.load('y.npy')
            X = np.concatenate((X, Xrelearning), axis=0)
            y = np.concatenate((y, Yrelearning), axis=0)
            features = np.concatenate((features, xrelearning), axis=0)
            labels = np.concatenate((labels, yrelearning), axis=0)
        ...
        model.save('emotion_result__.h5')
