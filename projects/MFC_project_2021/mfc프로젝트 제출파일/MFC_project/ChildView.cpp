...
//모든 음표를 지웠을때 현재페이지, 전체페이지 0으로 만들기
//음표가 아무것도 없을때 지우는 버튼을 울렀을때 메시지 나오게
// CChildView
...
void CChildView::draw_notes(int i) {//위치받는 리스트 필요
	...
	//오선지 밖에 나오면 줄 긋기(아래 위 둘다)
	if (clientview.Height() / 5 < clientview.Height() / 5 - (i - 1) * (gap / 2) || clientview.Height() / 5 - gap * 5 > clientview.Height() / 5 - (i - 3) * (gap / 2)) {
		dc.MoveTo(140 + (a * 35), clientview.Height() / 5 - (i - 3) * (gap / 3));
		dc.LineTo(160 + (a * 35)+ note, clientview.Height() / 5 - (i - 3) * (gap / 3));
	}
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)//피아노 눌렀을때 음이 무엇인지 출력, 음표그림
{
	...
	if (removeAll_button.PtInRect(point)) {//눌렀을때 전체 음표 지우기
		if (note_positions.IsEmpty()) {//지울 음표가 없으면 메시지/있으면 지우기
			AfxMessageBox(_T("지울 음표가 없습니다"));
		}
		else {
			...
			page = 0;//페이지 둘다 처음으로
			current_page = 0;//페이지 둘다 처음으로
		}
	}

	if (remove_button.PtInRect(point)) {//눌렀을때 음표 하나 지우기
		if (note_positions.IsEmpty()) {
			AfxMessageBox(_T("지울 음표가 없습니다"));
		}
		else {
