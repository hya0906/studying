...
class read_data:
    ...
    def txt_pir(self):
        ...
        if result_pir == "0":
            self.txt_pir_add = "지금은 사람이 없습니다. 소등 및 각종 디바이스들을 종료합니다. "
        else:
            self.txt_pir_add = "현재 시간 {}시 {}분,".format(hour,minute)
    ...
    def add_txt(self):
        result_pir = self.pir
        if result_pir == "0":
            self.txt = self.txt_pir_add
        else:
            self.txt = self.txt_pir_add + self.txt_decibel_add + self.txt_temp_add + self.txt_humi_add
