...
##        with keyboard.Listener(on_press = self.tts_on) as key:
        r = rec
        #여기서 process에서 사람있는지 없는지 확인하고 flag로 하고 여기로 받아와서 if else문으로 처리필요
        #일단 트리거가 먼저 필요할 것으로 확인됨
        while True:
            try:
            ...
            r.record_data() #◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆◆
            #print(f'TEMP: {self.temp}, HUMI: {self.humi}, DECIBEL: {self.peak}, PIR: {self.pir}, WAVE: {self.ultra}, LABELING RESULT: {self.machine.c}\n (from receive_data)')
            time.sleep(0.99)  # every 1min, records
