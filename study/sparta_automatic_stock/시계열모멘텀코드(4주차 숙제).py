...
class Kiwoom(QObject):
    ...
    def OnReceiveTrData(self, scrNo, rQName, trCode, recordName, prevNext, dataLength, errorCode, message, splmMsg):
        ...
        if rQName == "시가총액":
            ...
            if cnt > 12:
                close_1M = int(self.GetCommData(trCode, rQName, 1, "현재가"))
                close_12M = int(self.GetCommData(trCode, rQName, 12, "현재가"))

                if close_1M == 0:
                    self.li_stock[self.idx_li_stock]['시계열모멘텀'] = 0
                else:
                    self.li_stock[self.idx_li_stock]['시계열모멘텀'] = float(close_1M) / close_12M
            else:
                self.li_stock[self.idx_li_stock]['시계열모멘텀'] = 0

            print('[%d/%d] 종목명: %s 시계열모멘텀: %.2f' % (
                self.idx_li_stock + 1, len(self.li_stock), self.li_stock[self.idx_li_stock]['종목명'],
                self.li_stock[self.idx_li_stock]['시계열모멘텀']))
    ...
    @SyncRequestDecorator.kiwoom_sync_request
    def get_opt10083(self, code):
        # 오늘 날짜 가져오기
        from datetime import datetime
        date_today = datetime.today().strftime('%Y%m%d')

        self.SetInputValue("종목코드", code)
        self.SetInputValue("기준일자", date_today)
        self.SetInputValue("수정주가구분", "1")
        self.CommRqData("시계열모멘텀", "opt10083", 0, "0106")
...
class SpartaQuant(QMainWindow):
    ...
    def main_thread(self):
        ...
        # 100개씩 시가총액 정보 가져오기
        cnt_li_code_from_cond = len(self._kiwoom.li_code_from_cond)

        arrCode = []
        codeCount = 0

        while cnt_li_code_from_cond > 0:
            if cnt_li_code_from_cond >= 100:
                arrCode = self._kiwoom.li_code_from_cond[:100]
                codeCount = 100

                self._kiwoom.li_code_from_cond = self._kiwoom.li_code_from_cond[100:]
                cnt_li_code_from_cond = cnt_li_code_from_cond - 100
            else:
                arrCode = self._kiwoom.li_code_from_cond
                codeCount = len(arrCode)
                cnt_li_code_from_cond = 0
        ...
        # 시계열 정보 가져오기           #
        ###############################
        self._kiwoom.idx_li_stock = 0

        # opt10001을 통한 종목 재무재표 정보 가져오기
        for stock_item in self._kiwoom.li_stock:
            self._kiwoom.get_opt10083(stock_item['종목코드'])
        ...
        # 시계열모멘텀 순위 오름차순 정렬 ,등수 메기기
        self._kiwoom.li_stock = sorted(self._kiwoom.li_stock, key=lambda x: x['시계열모멘텀'], reverse=True)
        cnt = 1
        for stock_item in self._kiwoom.li_stock:
            stock_item['시계열모멘텀순위'] = cnt
            cnt = cnt + 1

        # 총합(PER순위 + PBR순위 + ROE순위 + 시계열모멘텀순위) 구하기
        for stock_item in self._kiwoom.li_stock:
            stock_item['총합'] = stock_item['PER순위'] + stock_item['PBR순위'] + stock_item['ROE순위'] +  + stock_item['시계열모멘텀순위']
        ...
        idx = 1
        for stock_item in self._kiwoom.li_stock:
            print("[%d/%d] 종목명: %s PER: %.2f PBR: %.2f 시계열모멘텀: %.2f PER순위: %d PBR순위: %d ROE순위: %d 시계열모멘텀순위: %d 총합: %d" % (
            idx, len(self._kiwoom.li_stock), stock_item['종목명'], stock_item['PER'], stock_item['PBR'], stock_item['시계열모멘텀'],
            stock_item['PER순위'], stock_item['PBR순위'], stock_item['ROE순위'], stock_item['시계열모멘텀순위'], stock_item['총합']))
            idx = idx + 1
