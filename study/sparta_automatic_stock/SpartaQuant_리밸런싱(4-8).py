...
class Kiwoom(QObject):
    ...
    cnt_code_from_cond = -1  # 조건식 결과종목 코드 갯수
    li_stock = []  # 주식종목정보 리스트
    idx_li_stock = 0  # li_stock 의 인덱스
    total_budget = 0  # 계좌잔고 내 총자산
    jango = {}  # 계좌잔고 내 주식종목 정보
    ...
    def OnReceiveTrCondition(self, scrNo, codeList, conditionName, index, next):
        ...
        self.cnt_code_from_cond = len(self.li_code_from_cond)
    ...
    def OnReceiveTrData(self, scrNo, rQName, trCode, recordName, prevNext, dataLength, errorCode, message, splmMsg):
        ...
        if rQName == "시가총액":
            ...
            for i in range(cnt):
                ...
                market_capital = int(self.GetCommData(trCode, rQName, i, "시가총액"))
                bid_price = int(
                    self.GetCommData(trCode, rQName, i, "매수5차호가").strip().replace('+', '').replace('-', ''))
                ask_price = int(
                    self.GetCommData(trCode, rQName, i, "매도5차호가").strip().replace('+', '').replace('-', ''))
                print("종목명:", name, "종목코드:", code, "시가총액:", format(market_capital, ','), "매수5차호가:",
                      format(bid_price, ','), "매도5차호가:",
                      format(ask_price, ','))
                self.cnt_code_from_cond = self.cnt_code_from_cond - 1

                dict_tmp = {}
                dict_tmp['종목명'] = name
                dict_tmp['종목코드'] = code
                dict_tmp['시가총액'] = market_capital
                dict_tmp['매수5차호가'] = bid_price
                dict_tmp['매도5차호가'] = ask_price
                dict_tmp['PER'] = 0
                dict_tmp['PBR'] = 0
                dict_tmp['ROE'] = 0
                dict_tmp['PER순위'] = 0
                ...
                dict_tmp['총합'] = 0
                self.li_stock.append(dict_tmp)

            # 모든 종목에 대한 시가총액 정보 획득 시 시가총액 하위 20% 선별
            if self.cnt_code_from_cond == 0:
                self.li_stock = sorted(self.li_stock, key=lambda x: x['시가총액'], reverse=False)
                len_li_stock = len(self.li_stock)
                cnt_cut = int(len_li_stock / 5)
                self.li_stock = self.li_stock[:cnt_cut]

            print("남은갯수:", self.cnt_code_from_cond)
            ...
            self.li_stock[self.idx_li_stock]['PER'] = PER
            self.li_stock[self.idx_li_stock]['PBR'] = PBR
            self.li_stock[self.idx_li_stock]['ROE'] = ROE
            print('[%d/%d] 종목명: %s 시가총액: %s PER: %.2f PBR: %.2f ROE: %.2f' % (
                self.idx_li_stock + 1, len(self.li_stock), name, self.li_stock[self.idx_li_stock]['시가총액'], PER, PBR,
                ROE))
            self.idx_li_stock = self.idx_li_stock + 1

        elif rQName == '잔고조회':
            # 잔고 정보 획득
            cnt = self.GetRepeatCnt(trCode, rQName)
            self.total_budget = int(self.GetCommData(trCode, rQName, 0, "추정예탁자산").strip())
            print('추정예탁자산: %s'%format(self.total_budget, ','))
            ...
            for i in range(cnt):
                ...
                dict_tmp = {}
                dict_tmp['종목코드'] = code
                dict_tmp['평가금액'] = estimated_amount
                dict_tmp['보유수량'] = count

                print('종목명: %s 평가금액: %s 보유수량: %s'%(name, format(estimated_amount, ','), format(count, ',')))

                self.jango[name] = dict_tmp
    ...
    @SyncRequestDecorator.kiwoom_sync_request
    def get_jango_data(self, prevNext="0", **kwargs):
...
class SpartaQuant(QMainWindow):
    ...
    def main_thread(self):
        ...
        # 100개씩 시가총액 정보 가져오기
        cnt = len(self._kiwoom.li_code_from_cond)
        ...
        while cnt > 0:
            if cnt >= 100:
                arrCode = self._kiwoom.li_code_from_cond[:100]
                codeCount = 100

                self._kiwoom.li_code_from_cond = self._kiwoom.li_code_from_cond[100:]
                cnt = cnt - 100
            else:
                arrCode = self._kiwoom.li_code_from_cond
                codeCount = len(arrCode)
                cnt = 0

            arrCode = ";".join(arrCode) + ';'

            self._kiwoom.CommKwRqData(arrCode, 0, codeCount, 0, "시가총액", "0122")

        # 시가총액 정보 가져오기 완료 대기
        while self._kiwoom.cnt_code_from_cond != 0:
            sleep(1)
        sleep(1)

        # 선정 된 시가총액 하위 20% 종목 출력
        print('+++ 시가총액총 하위 20% 선정 +++')
        idx = 1
        for stock_item in self._kiwoom.li_stock:
            print("[%d/%d] 종목명: %s 시가총액: %s" % (
            idx, len(self._kiwoom.li_stock), stock_item['종목명'], format(stock_item['시가총액'], ',')))
            idx = idx + 1
        ...
        # opt10001을 통한 종목 재무재표 정보 가져오기
        for stock_item in self._kiwoom.li_stock:
            self._kiwoom.get_opt10001(stock_item['종목코드'])

        # 종목 재무재표 정보 가져오기 완료 대기
        while len(self._kiwoom.li_stock) != self._kiwoom.idx_li_stock:
            sleep(1)
        sleep(1)
        ...
        # PER 순위 오름차순 정렬 및 등수 메기기
        self._kiwoom.li_stock = sorted(self._kiwoom.li_stock, key=lambda x: x['PER'], reverse=False)
        cnt = 1
        for stock_item in self._kiwoom.li_stock:
            stock_item['PER순위'] = cnt
            cnt = cnt + 1

        # PBR 순위 오름차순 정렬 ,등수 메기기
        self._kiwoom.li_stock = sorted(self._kiwoom.li_stock, key=lambda x: x['PBR'], reverse=False)
        cnt = 1
        for stock_item in self._kiwoom.li_stock:
            stock_item['PBR순위'] = cnt
            cnt = cnt + 1

        # ROE 순위 오름차순 정렬 ,등수 메기기
        self._kiwoom.li_stock = sorted(self._kiwoom.li_stock, key=lambda x: x['ROE'], reverse=True)
        cnt = 1
        for stock_item in self._kiwoom.li_stock:
            stock_item['ROE순위'] = cnt
            cnt = cnt + 1

        # 총합(PER순위 + PBR순위 + ROE순위) 구하기
        for stock_item in self._kiwoom.li_stock:
            stock_item['총합'] = stock_item['PER순위'] + stock_item['PBR순위'] + stock_item['ROE순위']

        # 총합기준 오름차순 정렬 및 최종종목 선정
        self._kiwoom.li_stock = sorted(self._kiwoom.li_stock, key=lambda x: x['총합'], reverse=False)
        self._kiwoom.li_stock = self._kiwoom.li_stock[:20]

        # 최종 선발 된 종목 출력
        print('+++ 최종 선정 종목 +++')
        idx = 1
        for stock_item in self._kiwoom.li_stock:
            print("[%d/%d] 종목명: %s PER: %.2f PBR: %.2f PER순위: %d PBR순위: %d ROE순위: %d 총합: %d" % (
            idx, len(self._kiwoom.li_stock), stock_item['종목명'], stock_item['PER'], stock_item['PBR'],
            stock_item['PER순위'], stock_item['PBR순위'], stock_item['ROE순위'], stock_item['총합']))
            idx = idx + 1

        ###############################
        # 6. 잔고 정보 받아오기          #
        ###############################
        self._kiwoom.get_jango_data()

        # 잔고 정보 받아오기 완료 대기
        while self._kiwoom.total_budget == 0:
            sleep(1)
        sleep(1)

        ##################################
        # 7. 총자산 대비 종목별 매수금액 선정 #
        ##################################
        print('총자산: ' + format(self._kiwoom.total_budget, ',') + '원')
        self._kiwoom.total_budget = int(self._kiwoom.total_budget * 0.9)  # 매매 시 현금여유분을 위해 총자산의 98%만 매매
        print('변경 된 총자산(90%): ' + format(self._kiwoom.total_budget, ',') + '원')
        money_per_stock = int(self._kiwoom.total_budget / 20)
        print('종목별매수금액: ' + format(money_per_stock, ',') + '원')

        ##################################
        # 8. 리밸런싱                     #
        ##################################

        input('리밸런싱을 시작하겠습니다.. 아무 키나 눌러주세요..')

        # 최초 매수
        if len(self._kiwoom.jango) == 0:
            for stock_item in self._kiwoom.li_stock:
                name = stock_item['종목명']
                code = stock_item['종목코드']
                price = stock_item['매도5차호가']
                count = int(money_per_stock / price)
                print("[매수] 종목명: %s 종목코드: %s 가격: %d 주식수: %d" % (name, code, price, count))

                # 시장가 매수(갯수: 종목별매수금액 / 매도5차호가)
                self._kiwoom.SendOrder("RQ_1", "0101", self._kiwoom.ACCOUNT, 1, code, count, 0, "03", "")
                sleep(1)
        # 리밸런싱
        else:
            ###############
            # 매도 진행     #
            ###############
            # 잔고에 있는 종목 리스트와 최종선별 된 종목 리스트 비교
            for stock_name_in_jango in self._kiwoom.jango.keys():
                # 최종선별 된 종목 리스트에 없는 잔고 내 종목 전액 매도
                if stock_name_in_jango not in [i['종목명'] for i in self._kiwoom.li_stock]:
                    code = self._kiwoom.jango[stock_name_in_jango]['종목코드']
                    count = self._kiwoom.jango[stock_name_in_jango]['보유수량']
                    print("[매도] 종목명: %s 종목코드: %s 주식수: %d" % (stock_name_in_jango, code, count))

                    # 종목 전체 보유량 시장가 매도
                    self._kiwoom.SendOrder("RQ_1", "0101", self._kiwoom.ACCOUNT, 2, code, count, 0, "03", "")
                    sleep(1)
                # 종목별매수금액 초과 주식 일부 매도
                elif self._kiwoom.jango[stock_name_in_jango]['평가금액'] > money_per_stock:
                    code = self._kiwoom.jango[stock_name_in_jango]['종목코드']
                    estimated_amount = self._kiwoom.jango[stock_name_in_jango]['평가금액']
                    price = [i['매수5차호가'] for i in self._kiwoom.li_stock if i['종목명'] == stock_name_in_jango][0]
                    count = 0

                    while estimated_amount - price * count > money_per_stock:
                        count = count + 1

                    # 종목 부분 보유량 시장가 매도
                    print("[부분매도] 종목명: %s 종목코드: %s 가격: %d 주식수: %d" % (stock_name_in_jango, code, price, count))
                    self._kiwoom.SendOrder("RQ_1", "0101", self._kiwoom.ACCOUNT, 2, code, count, 0, "03", "")
                    sleep(1)

            input('매도주문 완료 후 엔터키를 눌러주세요..')

            ###############
            # 매수 진행     #
            ###############
            # 최종선별 된 종목 리스트와 잔고에 있는 종목 리스트 비교
            for stock_item in self._kiwoom.li_stock:
                name = stock_item['종목명']
                code = stock_item['종목코드']
                price = stock_item['매도5차호가']

                # 최종선별 된 종목 리스트에 편입 된 종목 매수
                if name not in self._kiwoom.jango.keys():
                    count = int(money_per_stock / price)
                    print("[매수] 종목명: %s 종목코드: %s 가격: %d 주식수: %d" % (name, code, price, count))
                ...
                # 종목별매수금액 미달 주식 일부 매수
                    ...
                    estimated_amount = self._kiwoom.jango[name]['평가금액']
                    ...
                    if count > 0:
                        print("[부분매수] 종목명: %s 종목코드: %s 가격: %d 주식수: %d" % (name, code, price, count))

                        # 종목 추가 수량 시장가 매수(갯수: 종목별매수금액 / 매도5차호가)
                        self._kiwoom.SendOrder("RQ_1", "0101", self._kiwoom.ACCOUNT, 1, code, count, 0, "03", "")
                        sleep(1)
