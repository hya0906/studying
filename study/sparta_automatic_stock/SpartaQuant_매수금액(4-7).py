    ...
    def OnReceiveTrData(self, scrNo, rQName, trCode, recordName, prevNext, dataLength, errorCode, message, splmMsg,
        ...
        if rQName == "시가총액":
            ...
            for i in range(cnt):
                ...
                bid_price = int(self.GetCommData(trCode, rQName, i, "매수5차호가").strip().replace('+', '').replace('-', ''))
                ask_price = int(self.GetCommData(trCode, rQName, i, "매도5차호가").strip().replace('+', '').replace('-', ''))
                print("종목명:", name, "종목코드:", code, "시가총액:", format(market_capital, ','), "매수5차호가:",
                      format(bid_price, ','), "매도5차호가:", format(ask_price, ','))
